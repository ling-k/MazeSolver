...
print("Turning...")
command.angular.z = -0.5 * multi_rot
command.linear.x = 0.1 * multi
...
while not rospy.is_shutdown():
    ...
    while(near_wall == 0 and not rospy.is_shutdown()): #1
        ...
        if(min_front > 0.2 and min_right > 0.2 and min_left > 0.2):    
            command.angular.z = -0.1 * multi_rot    # if nothing near, go forward
            command.linear.x = 0.15 * multi
            print "C"
        elif(min_left < 0.2):           # if wall on left, start tracking
            near_wall = 1       
            print "A"            
        else:
            command.angular.z = -0.25 * multi_rot   # if not on left, turn right 
            command.linear.x = 0.0 * multi
        ...
        if(min_front > 0.2): #2
            if(min_left < 0.12):    #3
                ...
                command.angular.z = -1.2 * multi_rot
                command.linear.x = -0.1 * multi
            ...
            command.angular.z = -1.0 * multi_rot
            command.linear.x = 0.0 * multi
            cmd_vel_pub.publish(command)
